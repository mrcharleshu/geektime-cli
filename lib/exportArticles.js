...
module.exports = async function exportArticles(cid, options) {
  ...
  const coverFile = `cover.${columnCover.split('/').pop().split('.').pop()}`;

  if (!existsSync(path.join(staticDir, coverFile))) {
    await download(columnCover, staticDir, { filename: coverFile });
  }
